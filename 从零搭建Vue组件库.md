...
```vue
<template>
  <button class="wang-button" :class="btnClass">
    <slot></slot>  
  </button>
</template>
<script>
export default {
  ...
  computed: {
    btnClass() { // 动态添加按钮样式
      let classes = [];
      if (this.type) {
        classes.push(`wang-button-${this.type}`);
      }
      return classes;
    }
  },
...
```vue
<template>
  <svg class="wang-icon" aria-hidden="true">
...
```vue
<template>
  <button class="wang-button" :class="btnClass" :disabled="loading">
    <wang-icon :icon="icon" v-if="icon && !loading" class="icon"></wang-icon>
    <wang-icon icon="loading" v-if="loading" class="icon loading"></wang-icon>
    <span v-if="this.$slots.default">
      <slot></slot>
    </span>
  </button>
...
describe('button.vue', () => {
    it('1.测试slot是否能正常显示', () => {
        ...
        expect(wrapper.text()).to.equal('wang-ui')
    })
    it('2.测试传入icon属性', () => {
        const wrapper = shallowMount(Button, {
            stubs: {
...
```bash
"lib": "vue-cli-service build --target lib --name wang-ui  ./src/packages/index.js"
```

#### (2)配置运行入口

```bash
"main": "./dist/wang-ui.umd.min.js"
```
